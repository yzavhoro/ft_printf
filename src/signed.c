...
static void precount(s_additional_params *ap, ssize_t nb, t_flags *flags)
{
    if (nb < 0)
    {
    ...
    ap->pres -= ap->nb_len + ap->add_symb;
    CHECK_UNDERFLOW(ap->pres);
    ap->width -= ap->nb_len + ap->add_symb + ap->pres;
...
static void fill_str(char *str, t_additional_params *ap, ssize_t nb, t_flags *flags)
{
    ...
    if (ap->add_symb)
        *(str + offset - 1) = IS_SET(flags->out_fmt_flags, PLUS) ? '+' : ' ';
    if (ap->pres)
        memset(str + offset - ap->pres, '0', ap->pres);
    if (ap->width)
    {
